...
class PlatformPlugin extends PluginModel
{
    ...
    {
        return ecjia_config::getAddonConfig('platform_plugins', true, true);
    }
    ...
    {
        $data = $this->enabled()->orderBy('ext_id', 'desc')->get()->toArray();
        return $data;
    }
...
//     }
    ...
    /**
     * 取得已安装的扩展
     * @param   array   $available_plugins  可使用的插件，一维数组 ['mp_jfcx', 'mp_orders']
     * @return  array   已安装的扩展方式列表
     */
    public function getOnlineExtends(array $available_plugins = array())
    {
//         $model = $this->online();
//         if (!$include_balance) {
...
//         }
        ...
        $data = $model->select('ext_id', 'ext_code', 'ext_name')->get();
        
        $extend_list = array();
        	
        if (!empty($data)) {
            
            $extend_list = $data->mapWithKeys(function ($item) use ($available_plugins) {
                if (empty($available_plugins)) {
                    $available_plugins = array_keys($this->getInstalledPlugins());
                }
                
                if (in_array($item['ext_code'], $available_plugins)) {
                    return array($item);
                } else {
                    return array();
                }
     ...
     * @return  array   配送方式数组
     */
    public function getAvailableExtends(array $available_plugins = array(), $support_cod = true, $cod_fee = 0, $is_online = false)
    {
        $model = $this->enabled();
...
//         }
        ...
        $data = $model->select('ext_id', 'ext_code', 'ext_name')
        ...
        if (!empty($data)) {
            ...
            $pay_list = $data->mapWithKeys(function ($item) use ($available_plugins, $cod_fee) {
                ...
                if (in_array($item['ext_code'], $available_plugins)) {
                    $item['ext_name'] = $this->channel($item['ext_code'])->getDisplayName();
                    return array($item);
                } else {
                    return array();
                }
    ...
    {
        $data = $this->enabled()->orderBy('ext_id', 'desc')->first();
    ...
    public function channel($code = null)
    {
        ...
        if (empty($data)) {
            return new ecjia_error('plugin_not_found', $code . ' extend not found!');
        }
